...
func LoadClientResolvers(filename string) ([]ResolverAddress, map[string]int, error) {
	...
	for scanner.Scan() {
		...
		if !target.isPrefix {
			addResolver(&endpoints, resolverMap, target.addr.String(), port)
			continue
		}
		...
		for _, addr := range expandPrefixHosts(target.prefix) {
			addResolver(&endpoints, resolverMap, addr.String(), port)
		}
	}
...
func addResolver(endpoints *[]ResolverAddress, resolverMap map[string]int, ip string, port int) {
...
func expandPrefixHosts(prefix netip.Prefix) []netip.Addr {
	prefix = prefix.Masked()
	...
	addresses := make([]netip.Addr, 0, 16)
