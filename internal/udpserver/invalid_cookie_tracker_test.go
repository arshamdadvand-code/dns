...
func TestInvalidCookieTrackerEmitsOnlyAfterThreshold(t *testing.T) {
	...
	if tracker.Note(7, &expected, 22, now, 2*time.Second, 3) {
		t.Fatal("first invalid cookie attempt must not emit")
	}
	if tracker.Note(7, &expected, 22, now.Add(100*time.Millisecond), 2*time.Second, 3) {
		t.Fatal("second invalid cookie attempt must not emit")
	}
	if !tracker.Note(7, &expected, 22, now.Add(200*time.Millisecond), 2*time.Second, 3) {
		t.Fatal("third invalid cookie attempt must emit")
	}
	if tracker.Note(7, &expected, 22, now.Add(300*time.Millisecond), 2*time.Second, 3) {
...
func TestInvalidCookieTrackerCleanupRemovesExpiredAttempts(t *testing.T) {
	...
	if tracker.Note(7, &expected, 33, now, time.Second, 2) {
		t.Fatal("first invalid cookie attempt must not emit")
	}

	tracker.Cleanup(now.Add(2*time.Second), time.Second)
	if tracker.Note(7, &expected, 33, now.Add(2*time.Second), time.Second, 2) {
		t.Fatal("expired attempts must be cleaned before threshold is reached again")
	}
	if !tracker.Note(7, &expected, 33, now.Add(2100*time.Millisecond), time.Second, 2) {
